...
test.describe('Product Catalog & Search', () => {
  test('home page shows seed products', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'AtlasBook Air 14', level: 4 })).toBeVisible();
  });
  ...
  test('category filter updates product list', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Pulse ANC Kulaklik', level: 4 })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'SwiftCharge 65W', level: 4 })).toBeVisible();

    // Non-aksesuar products should not be visible
    await expect(page.getByRole('heading', { name: 'Nova X Pro', level: 4 })).not.toBeVisible();
  });
  ...
  test('product card navigates to detail page', async ({ page }) => {
    ...
    await page.getByRole('heading', { name: 'Nova X Pro', level: 4 }).click();
